...
class TestEmbeddingGenerator:
    ...
    def test_successful_model_loading(self, mock_sentence_transformer):
        ...
        mock_model.encode.return_value = [[0.1] * 384]  # Mock embedding with 384 dimensions
        mock_sentence_transformer.return_value = mock_model
        
        # Create embedding generator
        embedding_generator = EmbeddingGenerator("test-model")
        
        # Verify model was loaded correctly
        mock_sentence_transformer.assert_called_once_with("test-model")
        assert embedding_generator.model is not None
        assert embedding_generator.use_fallback is False
